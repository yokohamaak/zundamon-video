...
- 表情選択(`fExpression`)・ポーズ選択(`fPose`)は、シーン/話者と違いファイル実在チェックをしていない。
  ...
  流用する方針で検討する。
- 表情エディタに表情連動SE(`se-map.json`の`expression`カテゴリ)の編集UIを移設する。
  音(BGM/SE)画面のSE自動マッピングUIは2026-07-17に削除済み(表情連動以外は不要と判断)。
  データ本体とV2レンダラーの表情SE再生は生きているため、編集手段だけが失われている状態。
