...
[4] 動画化               cd video && npm run render        # 横16:9 → out/video.mp4
                           cd video && npm run render:short  # 縦9:16ショート(1ネタ) → out/short.mp4
```
...
横の `DialogueVideo` を `portrait` で縦化した `DialogueVideoShort`。**1ネタ＝1章**を切り抜く（既定=最初のtrivia章・60秒上限ガード）。

- **構成**: 上＝固定見出し（クリック率重視・黄/黒で字幕より目立たせ視線誘導）／中＝画像（主役・大きく）／下＝ライブ字幕。**キャラ立ち絵は出さない**。章バッジも出さない。
- **セーフゾーン**: 上はiPhone Dynamic Island、下〜26%はYouTube ShortsのUI（タイトル/説明/ボタン）を避けて配置。
- **固定見出し（hook）**: 章の `hook`（Geminスが生成・`script.json`の章メタ）。無ければタイトルから仮生成。**「さっき」等の参照を禁止した自己完結の一行**。`--meta-only`/[3] で meta 再生成すると反映。
- **終盤CTA**: 末尾約3.5秒に「続きは本編で／登録」をセーフゾーン内へ。`--props='{"ctaText":"..."}'` で上書き、`""` で非表示。
- **2.5Dパララックス**: 写真カットは深度マップで奥行きカメラ移動＝静止画を“動画らしく”動かす（下記）。ロゴ/スクショ（contain）は全体表示にフォールバック。

### 深度マップ生成（パララックス用・ローカル/無料）

```
pip install torch transformers pillow numpy   # 初回のみ（Mac/ローカル）
python main_story.py ... で動画素材を用意した後：
python make_depth.py --dir docs/story          # 各画像の <base>.depth.png を生成（Depth Anything V2・数秒/枚）
cd video && npm run dev                          # prep が depth-manifest.json を作り、縦ショートがパララックス適用
```

- 従量課金なし・全ローカル（初回だけモデルDL）。M1/MPSで数秒/枚。SVD等の動画生成より遥かに軽い。
- 深度が無い画像は通常のKen Burnsにフォールバック（必須ではない）。
