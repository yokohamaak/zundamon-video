...
### 作り方（レビューツールの `/shorts` ハブ、または CLI）

```
# 1) 本編の第N章を独立ショート台本へ生成（自己完結・掴み先頭・約40秒）＋画像取得まで→レビュー待ち
python main_story.py --from-script docs/story/script.json --short-from N --slug NAME --stop-after-images
# 2) 台本レビュー（/shorts の「台本レビュー」= 対象を docs/shorts/NAME に切替えた /story で編集。hookもここで編集）
python review_server.py --dir docs/story      # /shorts ハブから操作（生成・レビュー・書き出し）
# 3) 音声+meta（VOICEVOX）
python main_story.py --from-script docs/shorts/NAME/script.json --images-from-dir --output-dir docs/shorts/NAME
# 4) 任意: 深度生成（パララックス）   python make_depth.py --dir docs/shorts/NAME
# 5) 書き出し                         cd video && SRC_DIR=../docs/shorts/NAME npm run render:short
```
