...
function clampItemFlags(ci,flag,n){ (DATA.script||[]).forEach(t=>{
  if(t.chapter!==ci) return;
  if(Array.isArray(t[flag])){ const f=t[flag].filter(x=>x<n); if(f.length>1) t[flag]=f; else if(f.length===1) t[flag]=f[0]; else delete t[flag]; }
  else if(typeof t[flag]==='number' && t[flag]>=n) delete t[flag]; }); }
// panel_item を「この発言で出す項目」の集合として持つ（int or int配列）。チェックボックス的にkを付け外し。
// 1項目は1発言（同一kは他発言から外す）＝重複防止しつつ、1発言に複数項目を同時表示できる。
function togglePanelItem(ci,sid,k,tn){
  let arr = Array.isArray(tn.panel_item)?tn.panel_item.slice():(typeof tn.panel_item==='number'?[tn.panel_item]:[]);
  if(arr.includes(k)){ arr=arr.filter(x=>x!==k); }
  else { arr.push(k);
    (DATA.script||[]).forEach(t=>{ if(t.chapter===ci&&t.vizSeg===sid&&t!==tn){
      if(Array.isArray(t.panel_item)){ const f=t.panel_item.filter(x=>x!==k); if(f.length>1)t.panel_item=f; else if(f.length===1)t.panel_item=f[0]; else delete t.panel_item; }
      else if(t.panel_item===k){ delete t.panel_item; } }}); }
  arr.sort((a,b)=>a-b);
  if(arr.length>1) tn.panel_item=arr; else if(arr.length===1) tn.panel_item=arr[0]; else delete tn.panel_item;
  markDirty();
}
// 小道具
...
function turnHasViz(tn, ch){
  if(ch.panel) return tn.panel_event==='shrink'||typeof tn.panel_item==='number'||Array.isArray(tn.panel_item);
...
function turnVizControl(tn, gi, seg, ci){
  ...
  if(seg.panel){
    ...
    (seg.panel.items||[]).forEach((it,k)=>{ const cur=tn.panel_item; const on=Array.isArray(cur)?cur.includes(k):cur===k; const dis=(!ov && shrinkGi!=null && gi<shrinkGi);
      const b=document.createElement('button'); b.type='button'; b.className='vchip'+(on?' on':'')+(dis?' dis':''); b.textContent=(on?'☑':'☐')+'項目'+k;
      b.title=dis?'縮小より前には置けません（先に縮小を設定）':(it.text||'')+'（複数選択可＝この発言で同時表示）';
      b.onclick=()=>{ if(dis) return; togglePanelItem(ci,sid,k,tn); render(); }; ctrl.appendChild(b); });
  }
...
function renderRight(){
  ...
  const tabs=[['image','画像'],['viz',nseg?('大演出 '+nseg):'大演出'],['small','小演出'],['chapter','章']];
  ...
  tlr.appendChild(tll); tlr.appendChild(tli); smallPanel.appendChild(tlr);
  ...
  if(tn.telop){ const r2=ctlRow('テロップ'); sizeCtl(r2,'telopSize'); durCtl(r2,'telopDur'); smallPanel.appendChild(r2);
    const r3=ctlRow('色'); colCtl(r3,'文字','telopColor','#ffd84d'); colCtl(r3,'背景','telopBg','#121a28'); colCtl(r3,'枠','telopBorder','#ffd84d'); smallPanel.appendChild(r3); }
  if(tn.reaction){ const r2=ctlRow('リアクション'); sizeCtl(r2,'reactionSize'); durCtl(r2,'reactionDur'); smallPanel.appendChild(r2); }
  ...
  if(tn.telop||tn.reaction){ const pr=ctlRow('位置');
    ...
    const setPos=(x,y)=>{ if(placeMode==='reaction'&&tn.reaction){tn.reactionX=x;tn.reactionY=y;} else if(tn.telop){tn.telopX=x;tn.telopY=y;} else if(tn.reaction){tn.reactionX=x;tn.reactionY=y;} markDirty(); renderRight(); };
    pr.appendChild(vMini('中央へ',()=>setPos(0.5,0.5)));
    pr.appendChild(vMini('位置リセット',()=>{ ['telopX','telopY','reactionX','reactionY'].forEach(k=>delete tn[k]); markDirty(); renderRight(); }));
    smallPanel.appendChild(pr); }
  ...
  if(tn.telop||tn.reaction||punch||tn.effect==='flash'){
    ...
    smallPanel.appendChild(sp);
    const sn=document.createElement('div'); sn.style.cssText='font-size:10px;color:var(--sub);margin:0 0 8px'; sn.textContent='クリックで配置（上の「○を配置」で対象切替）。位置/大きさ/色/長さを反映・本番はポップ/バーストで動く。';
    smallPanel.appendChild(sn);
  }
  if(cur==='image') renderImageTab(r,tn,ch,ci);
  else if(cur==='viz') renderVizTab(r,tn,ch,ci);
  else if(cur==='small') r.appendChild(smallPanel);
