...
t('quiz: 新editorの設定にも文字サイズ調整(questionSize/answerSize)がある', () => {
  // 描画・metaは元から対応。欠けていた新editorの調整UIを補完したことの確認。
  assert.ok(/sizeField\('問いの大きさ',cfg\.quiz,'questionSize'\)/.test(html), '問いの大きさ調整');
  assert.ok(/sizeField\('答えの大きさ',cfg\.quiz,'answerSize'\)/.test(html), '答えの大きさ調整');
  assert.ok(/sl\.onchange=\(\)=>commit\(\)/.test(html), '確定時にだけ保存(setConfig)');
});
