...
t('既存行: 編集時に話者/表情セレクタを表示し即時反映・取消で復元', () => {
  ...
  // 確定はblurでなく行外クリック（プルダウンを押しても編集が終わらない）
  assert.ok(!/ta\.onblur=\(\)=>finish/.test(fn), 'blur確定は廃止');
  assert.ok(/const onDocDown=\(e\)=>\{ if\(!row\.contains\(e\.target\)\) finish\(false\); \}/.test(fn), '行外クリックで確定');
  assert.ok(/removeEventListener\('pointerdown', onDocDown, true\)/.test(fn), '確定時にリスナ解除');
});

t('BGM: 区間解決(初期=全体1本未設定/分割で後ろが新区間)・id採番', () => {
  const src = extractFn('_turnIndexMap') + '\n' + extractFn('nextBgmId') + '\n' + extractFn('bgmSpans');
  new Function('g', src + '\n;g._turnIndexMap=_turnIndexMap;g.nextBgmId=nextBgmId;g.bgmSpans=bgmSpans;')(globalThis);
  globalThis.DATA = { script: [{ id: 'turn-0001' }, { id: 'turn-0002' }, { id: 'turn-0003' }, { id: 'turn-0004' }] };
  let sp = bgmSpans();
  assert.equal(sp.length, 1); assert.equal(sp[0].seg, null); assert.deepEqual([sp[0].s, sp[0].e], [0, 3]);
  globalThis.DATA.bgmSegments = [{ id: 'bgm-0001', startTurnId: 'turn-0003', bgm: 'a.mp3' }];
  sp = bgmSpans();
  assert.equal(sp.length, 2);
  assert.equal(sp[0].seg, null); assert.deepEqual([sp[0].s, sp[0].e], [0, 1]);   // 先頭は暗黙の未設定
  assert.equal(sp[1].seg.bgm, 'a.mp3'); assert.deepEqual([sp[1].s, sp[1].e], [2, 3]);
  assert.equal(nextBgmId(), 'bgm-0002');
  assert.ok(/renderBgmTrack\(body\)/.test(html), 'BGM行を描画');
