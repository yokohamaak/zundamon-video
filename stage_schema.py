...
def _display_mode(value, path):
    ...
    if value["kind"] == "comic":
        ...
        if "bubble" in comic:
            ...
            if "align" in bubble and bubble["align"] not in {"right", "center", "left"}:
                _error(f"{path}.comic.bubble.align", "right/center/left のいずれかが必要です")
...
def validate_story_v2(data, scenes, mobs=None):
    ...
    for index, turn in enumerate(turns):
        ...
        if "transition" in turn and turn["transition"] not in {"cut", "fade-black", "fade-white", "crossfade", "wipe-left", "wipe-right", "slide-left", "slide-right"}:
            _error(f"{path}.transition", "cut / fade-black / fade-white / crossfade / wipe-left / wipe-right / slide-left / slide-right のいずれかが必要です")
