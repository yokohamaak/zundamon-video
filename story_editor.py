...
def _build_script_prompt(theme, length, notes, mode="safe",
    ...
    parts = [
        ...
        '- "cameraEffectSettings": {"zoom":{"amount":0.18,"duration":0.45},"pan":{"amount":0.08,"duration":0.4},"tilt":{"angle":2.8,"duration":0.35}} … cameraEffects の効き量をその行だけ上書きする（通常は不要）',
        '- "cameraCenter": {"x":0.50,"y":0.45} … その行から仮想カメラの中心点を明示指定する（未指定ならシーン既定）',
