...
def _build_script_prompt(theme, length, notes, mode="safe",
    ...
    parts = [
        ...
        '- "pose": "idle" / "cheer" / "recoil" / "lean" / "droop" / "flustered" / "proud" / "listening" / "sneak" / "wobble" / "point" … その行の話者ポーズ',
...
def _build_script_prompt_v2(theme, length, notes, mode="safe",
    ...
    parts = [
        ...
        '- "effects": {"voiceLines": {"x":0.5,"y":0.5,"side":"both"}} … 任意位置から漫画的な発声線を出す。PC・電話・画面外の声など「そこから音が出ている」時に使う。side は both/left/right。',
