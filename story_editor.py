...
def _build_script_prompt(theme, length, notes, mode="safe",
    ...
    example = (custom_example or "").strip() or (
        ...
        ' "scene": "FIRST", "expression": "normal", "pose": "proud" },\n'
    ...
    parts = [
        ...
        '- "pose": "idle" / "cheer" / "recoil" / "lean" / "droop" / "flustered" / "proud" / "listening" / "sneak" / "wobble" / "point" / "smartphone" / "thinking" … その行の話者ポーズ',
        ...
        '- "hideBubble": true … この行ではキャラクターと音声は残し、吹き出しだけ表示しない',
        '- text に "\\n" を入れない。吹き出し内改行や表示分割はツール側で調整する',
        ...
        ' … めたんがホワイトボードで3項目にまとめて解説するインサート。'
        ' このターンの speaker/text は通常通り扱われ(TTSも通常生成)、吹き出しだけ出ない'
...
def _build_script_prompt_v2(theme, length, notes, mode="safe",
    ...
    if adaptation:
        parts += [
            ...
            "- すでにあるセリフや言い回しはできるだけ維持し、V2で見やすくするためのターン分割・短い反応追加だけを行う。text 内改行は入れない。",
    ...
    parts += [
        ...
        "- 通常の1ターンは空白を除いて13〜36文字を中心にする。説明・宣言だけは37〜55文字まで許容するが、長台詞を連発しない。",
        "- text 内に改行（\\n）は入れない。吹き出し内の折り返しや分割はツール側・編集側で調整する。長い台詞を分けたい時は意味の切れ目で turn 自体を分ける。",
        ...
        "- text が空白だけのターン（空文字を含む）は、場面開始・時間経過・見せる間が必要な時だけにし、pause で尺を指定する。各イベントにつき1ターンまでにし、通常の会話を空ターンで水増ししない。",
        "",
        "━━━ 出力の全体構造（V2形式）━━━",
        ...
        '【pose】idle / cheer / recoil / lean / droop / flustered / proud / listening / sneak / wobble / point / smartphone / thinking',
        ...
        '- "se": [{"file":"se/alarm.mp3","at":0.0,"volume":0.9}] … この行だけ鳴らす手動SE',
        '- text に "\\n" を入れない。吹き出し内の改行や表示分割はツール側・編集側で調整する。別の吹き出しにしたい時は turn 自体を分ける。',
        "",
        "━━━ テロップ（caption・任意）━━━",
        ...
        '- "effects": {"reactionMark": {"targetId":"zundamon","mark":"!","anchor":"topRight","offsetX":0,"offsetY":0,"size":120,"motion":"pop","duration":0.85}} … キャラクターに！/？/汗/怒りなどのリアクションマークを出す。targetId 省略時は話者。anchor は topRight/topLeft/head/faceRight/faceLeft/screen、motion は pop/bounce/shake/none。',
        '- "effects": {"gloomShock": {"opacity":0.72,"lineCount":34,"lineOpacity":0.45,"color":"#071026"}} … 背景上・キャラ後ろに濃紺グラデーションと縦線を出すガーン演出（絶望・ドン引き・気まずい沈黙向け）。start/endで表示秒を絞れる。',
        ...
        '- 漫画: "displayMode": {"kind": "comic", "comic": {"image": "background/xxx.png", "bubble": {"type": "speech", "x": 0.3, "y": 0.25, "width": 0.32}, "camera": {"type": "zoomIn"}}}。1枚絵の上に吹き出しを重ねる漫画コマ表示。bubble.type は speech/thought/shout/narration、本文はそのターンの text。同じ絵を続けるターンには毎ターン同じ image を書き、吹き出しを残したい後続ターンは bubble に "keepPrevious": true を付ける。camera は fixed/zoomIn/zoomOut（省略可）。',
