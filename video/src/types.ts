...
export type Turn = {
  ...
  overlays?: { image: string; start: number; end: number; dir: "left" | "right" | "top" | "bottom" | "center"; size?: number; frame?: boolean }[];
...
export type Quiz = {
  ...
  textColor?: string;     // 問いの文字色（既定 #fff）
  ...
  boxWidth?: number;      // 問い土台/答えバナー共通の横幅（旧・フォールバック。画面比 0.3..1）
  questionBoxWidth?: number; // 問い土台の横幅（画面比 0.3..1・任意・無指定=1行に収まる幅へ自動）
  answerBoxWidth?: number;   // 答えバナーの横幅（同上）
