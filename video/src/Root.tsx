...
async function loadStory(): Promise<StoryVideoRouterProps> {
  ...
  const baseScenes = await (await fetch(staticFile("story-scenes.json"))).json() as SceneLibrary;
  // 台本専用のシーン上書き（story_editor の「シーン編集」で複製したシーンだけを持つ、任意ファイル）。
  // 同じシーンIDがあれば台本側を優先し、無ければ共通のままフォールバックする。
  let scenes = baseScenes;
  try {
    const storyScenesRes = await fetch(staticFile("story-01.scenes.json"));
    if (storyScenesRes.ok) {
      const storyScenes = await storyScenesRes.json() as SceneLibrary;
      scenes = {...baseScenes, scenes: {...baseScenes.scenes, ...storyScenes.scenes}};
    }
  } catch {
    // story-01.scenes.json 未配置時は共通シーンのみで描画継続
  }
