...
function renderCaption(text: string, turn: Turn | null | undefined, currentTime: number, unit?: {start: number; end: number} | null): React.ReactNode {
  ...
  return points.slice(0, -1).map((start, i) => {
    ...
    const candidate = effects.find((item) => item.start <= absolute && item.end > absolute);
    let fx = candidate;
    if (candidate && unit && text.length) {
      const duration = Math.max(0, unit.end - unit.start);
      const from = Math.max(0, candidate.start - base) / text.length;
      const until = Math.min(text.length, candidate.end - base) / text.length;
      const effectStart = unit.start + duration * from;
      const effectEnd = unit.start + duration * until;
      if (currentTime < effectStart || currentTime >= effectEnd) fx = undefined;
    }
    ...
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      {portrait ? (
        ...
        >
          ...
          <span style={{ color: "#fff", fontWeight: 700, fontSize: 24, textAlign: "center", lineHeight: 1.25, whiteSpace: "pre-line" }}>{quiz.question}</span>
        </div>
      ) : null}
...
export const DialogueVideo: React.FC<{
  ...
  let caption = "";
  let captionUnit: {text: string; start: number; end: number; emotion?: Emotion} | null = null;
    ...
    >
      ...
      {caption ? (
        ...
        >
          ...
          >
            {renderCaption(caption, activeTurn, t, captionUnit)}
          </div>
        </div>
      ) : null}
