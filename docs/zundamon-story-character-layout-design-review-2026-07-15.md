...
対象外: `legacy-dialogue/`。第1〜27節は調査時点の設計レビューであり、後続の実装状況は2026-07-15時点のv2実装を反映している。
...
このレビューで推奨した案2を、旧JSONのランタイム互換を持たない `schemaVersion: 2` として実装した。旧台本は必要時に外部で変換してv2 validatorへ通す運用であり、旧フィールドを新レンダラーで解釈する分岐は追加していない。
...
### 第1段階: 新スキーマと検証を決める — 実施済み

1. `schemaVersion`、instance、stage event、slot、framing、DisplayModeを `docs/story-stage-schema-v2.md` に確定し、`stage_schema.py` でv2のみを検証する。
2. 実在しないinstance/slot、同slot競合、`voiceOnly` 個体の描画、存在しないモブ表情素材、無効な構図指定を保存前に拒否する。
3. `test_stage_schema.py` と `video/test-fixtures/stage-v2-props.json` を用意した。旧スキーマのランタイム互換テストは持たない。
4. 旧台本は必要な時に外部で変換し、原本を残してvalidatorとプレビューで確認する。変換を自動実行・自動保存する機能は作らない。

### 第2段階: anchorをslotへ再定義する — 実施済み

1. `layouts.standard.slots`、slotのorigin/scale/z-index/camera preset/overlap可否をscene定義へ導入した。v2は旧`anchors`を使わない。
2. 台本エディタはslot、offset、手動配置、手動scale、z-indexを編集できる。scene editorはslotとcamera presetを事前設定する。
3. v2は `speakerAnchor` / `manualPos` / `focusSpeaker` / `manualCameraFrame` を保存・読込しない。
4. カメラは `sceneDefault` / `speaker` / `slot` / `manual` のframingと、非継承の`cameraMotion`へ分けた。手動構図はステージ全体図上の枠をドラッグ・リサイズして設定する。
5. 現実装の接地点は足元origin、ドラッグの掴み点は身体中央である。素材ごとの `stageOrigin` / `faceOrigin` メタデータは、座り姿・後ろ姿を導入する時まで保留する。

### 第3段階: 表示種別ごとのレイアウトを統一する — 必要範囲を実施済み

1. `standard`、`whiteboard`、`zunMeet` をv2の表示種別として実装した。ホワイトボードのpresenterとZunMeetの参加者はinstance IDを参照する。
2. 特殊表示中もstage eventは適用され、通常表示に戻ると解決済みの在席・配置へ復帰する。特殊表示へ通常stageの座標やカメラを流用しない。
3. 現在は新しい表示種別を増やさない。ニュースや再現VTRが必要になった時に、その表示専用のデータ契約・validator・描画を追加する。

### 第4段階: 必要になった機能だけ拡張する — 未実装
...
6. **人物モデルの初期範囲**: **実装済み。** 主役とモブを同じinstanceモデルへ載せ、異なる個体IDで同じ素材を複数回使える。`voiceOnly` は音声専用で画面へ出せない。
7. **登場・退場**: **決定済み。** 話者選択による暗黙登場は廃止し、登場・退場は明示イベントとして保存・編集する。
8. **手動配置の単位と操作点**: **部分実装。** 台本・slotは足元の論理接地点originを保存し、ドラッグの掴み点は身体中央に置く。これにより足元が画面外でも掴めない問題を避ける。素材別の `stageOrigin` / `faceOrigin`、座り姿などの接地点上書き、画面外の個体をキーボードで動かす操作は未実装である。
...
10. **表示種別の初期範囲**: **実装済み。** `standard`、`whiteboard`、`zunMeet` を移行した。新規表示種別は要求が出るまで追加しない。
11. **変換運用**: **決定済み。** 旧台本はChatGPT等で都度変換し、原本を残す。変換後JSONは `stage_schema.py` のvalidatorを通過してから保存・レンダリングする。
...
| 新モデルが重すぎる | instance複数化までは実装済み。自動3人構図・高度なanimationは必要になるまで実装せず、slot + offset/manualの予測可能性を優先する。 |
...
以下はv2のコード経路を実装したが、実素材・実フレームで網羅確認していない。追加実装の判断前、または素材追加時にfixtureと静止画で確認が必要である。

1. 全身・後ろ姿・座り姿の素材が増えた時、表情/pose/向きの組合せをどの能力表で許可するか。
2. v2の`whiteboard` / `zunMeet`を、実際の長編台本と実素材で各パターン確認できているか。
3. 現在編集中の旧`story-01.json`で、v2変換時に人の判断を要する旧anchor・手動座標・enter/exitがどれだけあるか。
4. ニュース表示・ミニ再現VTRを、全画面置換にするか、通常stage上の専用レイアウトにするかというプロダクト判断。
5. 画面外話者を「声のみ」で見せる需要と、ナレーションとのUI上の区別。
...
- **当初の最優先3点**: (1) v2 validator、(2) 差分から解決するstage state、(3) anchorをカメラ意味から切り離すslot契約は実装済み。
- **次に必要になった時に決めること**: 後ろ姿・座り姿の素材能力、登場・退場アニメーションの時間・補間契約、3人以上の自動構図と前後レイヤー。いずれも現時点では要求がないため実装しない。

---

# 追補: モブキャラ編集・シーンエディタのレビュー（2026-07-15 追記）

本編（1〜22節）は台本編集とレンダリングを対象とした。ここでは関連ツールである **モブキャラ定義編集**（`story_editor.html` 「モブキャラ」タブ + `story_editor.py /api/mobs`）と **シーンエディタ**（`scene_editor.html` / `scene_editor.py`）を、同じ観点で追加調査した結果を示す。第23〜27節は調査結果であり、v2実装後の反映状況は次節に記す。
...
- **調査時点では、モブ編集・シーンエディタ単体は1〜2人＋少数モブの現行運用で実用的だった**。特にカメラ3枠のWYSIWYGは良くできていた。
- ただし調査時点の **3ツール（台本・モブ・シーン）が参照整合性を持たず、配置ロジックが三重実装** である点は、本編で指摘した「配置・カメラ・表示種別の責務混在」を運用面でさらに増幅していた。
- v2では新規データの配置・カメラをslot契約へ移した。残る旧形式の参照置換と、シーンエディタをターン単位のWYSIWYGにするかは、28節の境界を維持して必要時に判断する。

## 28. モブ編集・シーンエディタへのv2反映状況

- **モブ**: v2では主役と同じinstanceモデルに載せる。`characterId` がモブ定義を参照し、同じモブ素材を別のinstance IDで複数配置できる。エディタとvalidatorは、そのモブに実在する口閉じ／口開き画像の組だけを表情候補として扱い、存在しない表情を選んで描画時に別表情へ丸める経路をなくした。
- **シーンエディタ**: v2 sceneは `layouts.standard.slots` と `cameraPresets` を編集する。ここは「台本ごとの人物配置」ではなく、背景に対する再利用可能な立ち位置と構図の事前設定を作る場所である。台本エディタ側でinstance、slot選択、offset、手動配置、framingを決めるため、責務が分かれた。
- **残る境界**: モブ定義のリネーム／削除時に、既存の旧台本・旧sceneを横断して自動置換する機能は作っていない。v2 validatorは変換後のデータ整合性を確認するが、旧形式を自動修復しない。シーンエディタも個別turnのstage eventを再生するWYSIWYGではなく、slot/camera presetの設計画面に留める。
- **未対応素材**: v2の`face`は `left` / `right` のみで、後ろ姿や座り姿を意味しない。モブを含め、素材の表情・ポーズ・向きの組合せを宣言する能力モデルは、対応素材が揃った時点で追加する。
