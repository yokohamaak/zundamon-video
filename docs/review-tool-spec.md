...
`review_server.py`（3576行）の単体仕様。**各操作とDOM/状態/API/永続キーの対応**を網羅する。UI/UX改善の作業仕様書として使う。

- 全体像・他ドキュメント：[`architecture.md`](architecture.md) / [`DEVELOPER_GUIDE.md`](DEVELOPER_GUIDE.md)
- 行番号は参照アンカー（実装変更で前後しうる・関数名で追うのが確実）。
- **方針**：Python標準ライブラリのみ（Flask等不使用）。HTML/CSS/JSはすべてPython文字列定数として埋め込み。外部Webフレームワークは足さない。
...
| `/story` | `STORY_PAGE` (1376–3219) | ★メイン review/edit UI |
| `/read` | `READ_PAGE` (3232) | 読み専用ファクトチェック |
...
2ペイン（左=台本アウトライン、右=画像/演出エディタ）。右ペインは sticky、`.wide` で全幅オーバーレイ化。
...
└─ .tp（2ペイングリッド）
   ...
   └─ #rpane.tp-right[.wide]（sticky・右ペイン）
      ├─ .rwbar（全幅トグル .rwbtn）
      ├─ テロップ/リアクション編集（選択発言に対して）
      └─ .rtabs（タブ：画像 / 演出 / 章）→ 各タブのパネル
```

### クライアント状態（グローバルJS変数・1606付近で宣言）

| 変数 | 型 | 内容 | 初期化 | 変更契機 |
|---|---|---|---|---|
| `DATA` | obj | `{theme, chapters[], script[]}` ＝台本全体 | 起動Promise.all (3217) | 全ての台本編集 |
...
| `dirty` | bool | 未保存フラグ | false | `markDirty()` (2706) |
...
```
Promise.all([ GET /api/script, GET /api/cuts, GET /api/status ])  (3217)
...
- **演出の時刻指定は「発言にアンカーを立てる」間接モデル**。直感的なタイムライン編集は無く、`build_chapter_topics` が後で秒へ解決する。WYSIWYGプレビューはHTML/CSSモック（Remotion非埋め込み）。
- **`renderLegacy()` (2510) は未使用の旧アコーディオン版**（参考保存）。現役は二画面 `render()` (2765)。改修時は混同しない。
- **全UIが単一Pythonファイルの文字列定数**。コンポーネント分割やビルド工程は無い。CSS変数 `_BASE_CSS` が唯一の共通スタイル源。
- 改善は `_BASE_CSS`（テーマ）→ `STORY_PAGE` の各 `render*` 関数（構造）→ 必要なら新APIの順で、**`meta.json` の契約と `review.json` スキーマを壊さない**範囲で進めるのが安全。
