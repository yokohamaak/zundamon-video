...
- `wipe-left`: 切替先ターンの画面を左からワイプ表示する。
- `wipe-right`: 切替先ターンの画面を右からワイプ表示する。
- `slide-left`: 切替先ターンの画面を左からスライド表示する。
- `slide-right`: 切替先ターンの画面を右からスライド表示する。

V2のwipe/slideは、旧V1の「次シーン先出し」ではなく、切替先ターンの画面全体を黒背景上に表示する方式である。背景・前景・人物・特殊表示をまとめた二重プレート遷移が必要になった場合は別途設計する。
...
実装済みの表示種別は `standard`、`whiteboard`、`zunMeet` だけである。新しい表示種別は要求が出るまで追加しない。`face` は `left` / `right` のみで、後ろ姿・座り姿・素材ごとの対応表は未実装である。登場・退場は状態イベントとして即時に反映され、時間・補間を持つ登場／退場アニメーションはまだ定義しない。
