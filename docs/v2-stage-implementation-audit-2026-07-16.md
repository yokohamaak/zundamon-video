...
P1は対応済み。以下の優先度はP2以降に引き継ぐ。

- **P1-1:** v2 scene editorのslot確認用キャラ候補にモブを追加した。モブは`mob:<mobId>`として主役IDと衝突しない形で保存する。これはscene editorのプレビュー専用値であり、台本の`instance`や配置状態には影響しない。足元基準、scale、z-index、身体中央のドラッグ点は主役プレビューと同じslot契約を使う。
- **P1-2:** モブの向きは`face`を最優先し、次に個体の`flip`、最後にモブ素材の既定flip（未指定なら通常slotの左右）を使うよう統一した。
- **P1-3 / P1-4:** モブ画像をホワイトボードの解説役とZunMeet参加者にも描画するようにした。通常表示と同じ表情画像／口パク用open画像を使う。
- **P1-5:** 新規v2 sceneは`figure: "full"`を持つ。scene editorから既存sceneの`full`/`bust`も明示的に変更でき、validatorも2値以外を拒否する。
- **P1-6:** scene editorで任意のcamera presetを追加・名前変更・削除できる。slotが参照中のpresetは削除できず、名前変更時は参照を更新する。

P2は対応済み。

- **P2-1:** `cameraMotion.shake`を通常stageの描画変形へ接続した。`strength`は画面上の最大揺れ幅（px）、`duration`はターン開始からの秒数で、揺れ幅はその区間内で減衰する。既存のshake SE発火とも同じ設定を共有する。
- **P2-2:** 基本タブのカメラ演出に`tilt`、`shake（px）`、`shake時間（秒）`を追加した。tiltはRemotionと同じ変換順で配置マーカーとドラッグ座標にも反映するため、手動配置時にプレビューと書き出しの座標系がずれない。
- **P2-3:** v2 validatorはstage event、instance patch、camera motion、display mode、scene slotの未知キーを拒否する。stage個体の`characterId`は主役素材または`mobs.json`の素材が存在する場合だけ許可し、`allowOverlap`はbooleanだけを許可する。
- **P2-4:** v2 camera frameをscene editorと同じ`width: 0.35〜1.0`、かつ枠が画面内に収まる`cx/cy`に制限した。手動構図UIも同じ下限へ揃えた。
- **P2-5:** 同一sceneのsmooth接続で音声時刻が未生成なら、基本タブに「音声生成後に有効」と表示する。
...
- `python3 test_stage_schema.py` — 21件成功
...
- `npx remotion still ... --props=video/test-fixtures/stage-v2-props.json` — camera motion（tilt/shake）を含むv2 fixtureの静止画成功
