...
expression?(expressions.jsonのキー), enter?([charId...]), exit?([charId...]), exitDir?("left"|"right"),
face?({charId:"left"|"right"}), emphasis?(bool=話者ズーム), shake?(bool), flashback?(bool),
telop?(str 例"― 前日 ―"), pause?(秒), insert?(StoryInsert), se?([{file,at?,volume?}]),
...
- 5タブ: **台本 / シーン(iframe:8770) / 表情(iframe:8772) / 台本生成 / 音(BGM/SE)**。
- 中央=Remotion Playerプレビュー(`/story-player.js`)。プレビュー上に**ズーム**(フィット/100-400%)。
- プレビュー下に**タイムライン(canvas)**: 上=セリフ(話者色), 下=BGM。クリックでシーク・再生ヘッド追従・
  横ズーム(フィット/2-16x)。**BGMレーンはD&D編集**(空きドラッグ=区間作成/本体=移動/端=リサイズ・
  0.05秒スナップ)。直下の「選択区間エディタ」でファイル/音量/start-end/フェード/削除/試聴▶。隙間=無音。
- 右=詳細(基本タブ: 話者/セリフ/シーン/表情/演出/enter-exit/手動SE、インサートタブ)。
