...
現時点で編集者目線の不足として残っている最重要項目は、**旧演出系の扱い方針**、**V2用一括編集の再設計**、**吹き出し仕様の再整理**である。登場退場方向は `stage.enter/exit[].animation.direction` として対応済み。
...
1. テロップをV2へ戻すか、Overlayへ統合するか。
2. 旧演出系のうち、最初に戻すものを選ぶか。
3. V2用一括編集で必要な対象を決めるか。
4. 吹き出し最大文字数・横幅・段数の仕様を再定義するか。
