...
現時点で編集者目線の不足として残っている最重要項目は、**旧演出系の扱い方針**、**V2用一括編集の再設計**、**吹き出し仕様の再整理**である。登場退場方向は `stage.enter/exit[].animation.direction`、テロップは `caption` として対応済み。
