...
├── review_server.py        # ★レビューUI（stdlibのみのローカルWeb・3576行）
...
## 3. UI構成（制作ツール ＝ `review_server.py`）

画像取得後に人が確認・差し替えするローカルWebツール。**Python標準ライブラリのみ**（Flask等不使用）。HTML/CSS/JSはすべてPython文字列定数として**埋め込み**。
...
### `/story` の構造（行 1376–3219 が本体）

2ペイン（左 = 台本、右 = 画像/演出エディタ・幅480px・モバイルで1ペイン化）。ダークテーマ。
...
| **1376–3219** | **`STORY_PAGE`（メインUI）** |
| 3232–3344 | `READ_PAGE` |
| 3347–3576 | HTTPハンドラ（`do_GET`/`do_POST`）と `main()` |

> プレビューはRemotion埋め込みではなく **HTML/CSSで本番レイアウトを再現したWYSIWYGモック**（全画面1920基準・中央=画像エリア基準で位置/大きさを本番一致させる）。
...
| レビューUIの見た目・操作 | `review_server.py` の `STORY_PAGE`（1376–3219）＋ `_BASE_CSS` |
...
- **レビューUIは stdlib のみ**。`review_server.py` に外部Webフレームワークを足さない方針（HTML/CSS/JSは埋め込み文字列）。
...
5. `review_server.py` の `STORY_PAGE`（1376–3219）… 制作ツールUI
